...
preferences {
	section("Shower Sensor") {
		input "sensorShower", "capability.motionSensor", multiple: true
	}
    
    section("Outside Sensor") {
		input "sensorOutside", "capability.motionSensor", multiple: true
	}
    
	section("Initial Lights") {
		input "lightsInit", "capability.switch", multiple: true
	}
    
	section("Time to stay on with no motion in seconds (30s)") {
		input "noMotionLimit", "number", title: "Number of seconds", required: false
	}
...
def initialize() {
    ...
    //state.sensors = [];
    //state.sensors.push(settings.sensorOutside)
    //state.sensors.push(settings.sensorShower)
   
    //log.debug(settings.sensorShower)
    //log.debug(state.sensors)
    

    subscribe(settings.sensorOutside, "motion.active", motionDetectedHandler)
    subscribe(settings.sensorOutside, "motion.inactive", motionStoppedHandler)
...
def motionStoppedHandler(evt) {
    ...
    runIn(settings.noMotionLimit, state.lightsInit.off())
}
